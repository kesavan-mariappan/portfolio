...
const Analytics = ({ onBack }) => {
  ...
  useEffect(() => {
    const unsubPV = onSnapshot(doc(db, 'analytics', 'pageViews'), (snap) => {
      setPageViews(snap.data()?.count || 0);
    });
    const unsubSC = onSnapshot(doc(db, 'analytics', 'sectionClicks'), (snap) => {
      setSectionClicks(snap.data() || {});
      setLoading(false);
    });
    return () => { unsubPV(); unsubSC(); };
  }, []);

  const totalClicks = Object.values(sectionClicks).reduce((a, b) => a + b, 0);
        ...
        {loading ? (
          ...
          <>
            <motion.div
              ...
              className="bg-gradient-to-br from-cyan-500/10 to-blue-600/10 border border-cyan-500/20 rounded-2xl p-8 mb-8 text-center"
            >
            ...
            <div className="space-y-4">
              <h2 className="text-xl font-semibold text-gray-300 mb-6">Section Clicks</h2>
                  ...
                  <motion.div
                    ...
                    className={`border rounded-xl p-5 ${
                      isResume
                        ...
                        : 'bg-gray-900/60 border-cyan-500/10'
                    }`}
                  >
                    ...
                    <div className="text-xs text-gray-500 mt-1">{pct.toFixed(1)}% of total clicks</div>
                  </motion.div>
                );
              })}
            </div>
